...
Live archive acceptance exposed a missing native Codex session reference and a nested Herdr metadata adapter mismatch. See [Herdr native session binding](herdr-session-binding.md): the adapter correction requires review; absent Codex metadata still blocks retained-session continuation. Neither the private owned-process diagnostic nor installing a future SessionStart hook authorizes replacing or resuming that process.
