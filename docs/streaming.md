...
## Integration with recovery and periodic review

Attempt-specific journals use the recovery generation in filenames and item keys. Journals are fsynced before replacement; recovery imports only the proven-dead attempt's validated public journal. Old attempt credentials remain denied by the shared runtime generation fence. Recovered responses use SSE IDs `<response-id>:g<generation>:<sequence>`; initial generation retains the original format. A cursor from a prior attempt is rejected, and a stream closes if its generation changes. After explicitly authorized recovery, attach without an old cursor to rebuild the current response state. Progress transport failures retain the private journal but cannot prevent the confirmed final reply from completing. The existing ten-minute chat timeout, three-minute background budget, durable begin/completion protocol, periodic-review silence and user-bus fallback remain intact.
