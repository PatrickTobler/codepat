...
The CLI and runtime can find an organization member and send a native Sokosumi Direct as the configured coworker. Sokosumi is the primary channel when a user asks CodePat to contact someone. There is no email fallback; obtain explicit user direction before using another channel.
...
2. Prepare a private registry using `deploy/contact-accounts.example.json`. Each entry maps the exact user ID + organization ID to an absolute owner-account file path. Exactly one entry must match a request.
...
The new runtime renders the updated `CODEPAT.md` into its coordinator directory. In a fresh active requester turn, run `contacts` for the intended recipient, verify identity and organization, write the already-authorized text to a private file, and call `dm-send` once with a stable key. Record its message/room IDs only if `accepted`; query the same key after a lost response. A queued/sending/uncertain result is not completion. Verify any notification/reading independently rather than claiming it from HTTP creation.
