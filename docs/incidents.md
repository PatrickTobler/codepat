...
The service rejects transitions to the current status. CodePat omits a redundant status observed during `task-report` preparation, but cannot eliminate changes between that read and the API write. A rejected transition must be reconciled, not silently converted into a comment-only retry. The scoped historical review found HTTP 422 records without retained response bodies: the exact original cause remains unverified. New diagnostics do not retroactively establish it or repair those records.
