...
## Delivery review and rejected task events

`task-report` returns a stable `notificationId` plus current `status`, `accepted`, POST `httpStatus`/`rejectionKind` and separate `reconciliationHttpStatus`. `ok` acknowledges a pending/sending/accepted reservation; failed, uncertain, superseded or missing reservations return false. `accepted` is true only after API acceptance, never proof of reading or task completion. Repeating the same job/task/status/text reuses its durable receipt, including concurrent requests, process restart and an uncertain first delivery. A queued acknowledgment is not an upstream acceptance. Distinct updates and distinct jobs remain separate reports. Receipts are additive JSON state and must be preserved with outbox backups. Older reports have no retroactive receipts; never replay them to manufacture one.

Periodic review scope validation runs before `incident-report` as well as other control operations. A review reserved under an earlier owner or organization cannot send or acknowledge incidents for a changed scope.

The live [task-event contract](https://api.sokosumi.com/v1/openapi.json) supports HTTP 422 rejections. CodePat retains the HTTP status and only these recognized diagnostic categories from a bounded response body: `same_status`, `queued_requires_schedule`, `insufficient_balance`. Other bodies remain unknown; raw response messages and data are not retained. The insufficient-balance contract can return a committed pause event even though the requested operation was rejected. Neither a failed local delivery nor HTTP 422 alone proves that all remote state was untouched. No category triggers automatic retry or status alteration.

The service rejects transitions to the current status. CodePat omits a redundant status observed during `task-report` preparation, but cannot eliminate changes between that read and the API write. Distinct pending reports are ordered behind unresolved earlier reports for that task. Immediately before a new report POST, a scoped GET checks actual status and assignment: an already-current requested status is omitted while its comment is preserved. If a confirmed `same_status` 422 still rejects a status-bearing POST, the same durable receipt becomes a comment-only pending delivery and retries after a fresh preflight; an uncertain POST is never replayed. Failed predecessors are not assumed accepted. An uncertain predecessor blocks subsequent report dispatch. The scoped historical review found HTTP 422 records without retained response bodies: the exact original cause remains unverified. New diagnostics do not retroactively establish it or repair those records.

POST diagnostics are updated only by the POST attempt. History/preflight GET failures and local reconciliation failures cannot relabel that outcome. Retryable pending 429s do not create autonomous failure incidents. When a source delivery is confirmed accepted, existing failure incidents are marked `resolvedAt` and removed from notification selection. Unattempted pending notices solely about resolved incidents are superseded; uncertain/sending notices are preserved and remain visible with resolution metadata. Reporting resolution does not replay any notification. Task report text is preserved in full, including failure/blocked reports; no silent 4,000-character truncation is applied.
