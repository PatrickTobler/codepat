...
The service rejects transitions to the current status. CodePat omits a redundant status observed during `task-report` preparation, but cannot eliminate changes between that read and the API write. Distinct pending reports are ordered behind unresolved earlier reports for that task. Immediately before a new report POST, a scoped GET checks actual status and assignment: an already-current requested status is omitted while its comment is preserved. Failed predecessors are not assumed accepted. An uncertain predecessor blocks subsequent report dispatch. A rejected POST remains terminal and must be reconciled, not silently converted into a comment-only retry. The scoped historical review found HTTP 422 records without retained response bodies: the exact original cause remains unverified. New diagnostics do not retroactively establish it or repair those records.
