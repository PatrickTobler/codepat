...
A chat worker first reserves an operation record, creates a Sokosumi task owned by the requesting user in their organization with the selected project and CodePat assignment, then prepares its Git worktree and Herdr tab. Existing task intake reuses the task ID. Supported worker kinds are `codex` (default) and `claude`; unsupported values fail before task creation. The chosen kind survives retries and follow-ups. Codex workers launch with `-s danger-full-access -a never`; Claude uses `--permission-mode acceptEdits`. CodePat may approve a routine, task-authorized dialog through an exact worker-scoped operation after inspecting the action; consequential or out-of-scope actions still require the requester. The coordinator is always Codex.

`send` steers an owned unheld busy worker. New sends to held/blocked workers are rejected; existing queued messages are retained. `stop` interrupts only a verified owned pane and invalidates its old reporting generation. `resume` retries the same repaired worker, preserving task/worktree identity. `read` retrieves recent owned output (or the saved result for archived workers). A terminal/idle Herdr state is not proof of completion: the worker must submit `worker-result`, and the coordinator must review evidence and post `task-report`. Sibling unfinished workers prevent a COMPLETED task report.

For a blocked pane, the active owner chat or a coordinator-claimed, worker-owned event turn may use `worker-approve-routine <worker> --file approval.json` only after `record-worker-hold` captured the exact owned dialog. Worker reporting credentials remain reporting-only. The private evidence must contain `routine: true`, category `read-only`, `tests`, or `dependency-install`, key `enter`, a SHA-256 `actionDigest`, bounded `actionText`, and a SHA-256 `dialogFingerprint` matching a fresh sanitized pane read. At record time CodePat reads the exact pane and derives the action binding; a caller-supplied action that differs is refused. Fresh holds record canonical action-text provenance version 1; missing, unknown, or older versions are refused and cannot be silently re-recorded. The recognized provider layouts are a trailing single dialog block with one `Action:`, `Command:`, or `Request:` line directly followed by the prompt, or one Claude Bash command box (`Bash command` plus box-prefixed command lines, optionally enclosed by matching box-drawing borders) followed by `Do you want to proceed?` and exactly one selected option (`❯`, `>`, or `›`, with optional numbering). Shell continuations, interior blank lines, and every command-significant line in a recognized box are included in the action binding; quoted/internal whitespace and ASCII shell pipes are preserved, and unrecognized lines or boundaries fail closed. Only exact one-time labels (`Yes`, `Allow`, `Approve`, `Continue`, `Run`, or `Accept`) are eligible; persistent/session/project/all/future/remember options, stale or multiple blocks, and unknown layouts fail closed. CodePat rechecks the real caller job, task owner, organization, generation, pane and blocked status, serializes the worker operation, stores a durable receipt before sending the key, and treats a transport error as uncertain (never replay it). Historical boolean-only holds, denied/cancelled decisions, unknown dialogs and consequential actions remain blocked for explicit reconciliation. Periodic review and incident scopes cannot approve dialogs; they remain notification-only because no worker-event approval turn is created for a block.

The current Claude gutter layout uses the stricter [owner-chat routine flow and live acceptance procedure](routine-recovery-acceptance.md), including structured session binding, exact recorded fingerprint, one-time choice 1, a second live read, durable key receipts and explicit closed-dialog reconciliation. Its new owner-only flow is not enabled by a worker event.
