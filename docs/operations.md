...
Every minute, CodePat checks locally known task conversations against Sokosumi. After a five-minute grace period it queues an inspection when a READY or RUNNING task has no working resource or queued turn, or when an instruction has no recorded disposition. Two inspection attempts are allowed per incident. Persistent failures generate one task comment and remain visible in `status.taskHealth`. Queued tasks with a stale runner heartbeat generate one visible alert while preserving their original job; they are not duplicated. These checks inspect preserved state; they never replay worker actions themselves. Unknown tasks that have never reached this bridge remain dependent on the Sokosumi event feed. External resources need an explicit AWAITING_EXTERNAL state and coordinator follow-up.
