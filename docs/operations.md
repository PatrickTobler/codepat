...
For a blocked pane, the active owner chat or the worker's own generation-scoped event turn may use `worker-approve-routine <worker> --file approval.json` only after `record-worker-hold` captured the exact owned dialog. The private evidence must contain `routine: true`, category `read-only`, `tests`, or `dependency-install`, key `enter`, a SHA-256 `actionDigest`, a SHA-256 `dialogFingerprint`, and a bounded `authorizationReference`. CodePat rechecks the task owner, organization, generation, pane and blocked status, stores a durable receipt before sending the key, and treats a transport error as uncertain (never replay it). Historical boolean-only holds, denied/cancelled decisions, unknown dialogs and consequential actions remain blocked for explicit reconciliation. Periodic review and unrelated worker scopes cannot approve dialogs; the worker event credential can approve only its exact worker generation.
