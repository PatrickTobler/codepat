...
function fixture(t: TestContext) {
  ...
  const runtime = new Runtime(state, { call: async () => ({}), agents: async () => [] }, { dataDir: dir, cliPath: "cli", repo: dir, apiUrl: "http://localhost:1" });
...
test("stale completion attempts are rejected after recovery rotates the generation", async t => {
  const f = fixture(t);
  reconcileDeadTurn(f.runtime, f.dir, f.job.id, dead);
  assert.equal(f.runtime.job(f.job.id).status, "queued");
  f.runtime.nextJob("replacement", 1);
...
test("chat and every background turn use validated independent hour budgets", () => {
  ...
  for (const kind of ["chat", "task", "future-background-kind"]) {
  ...
  assert.equal(turnDeadlines("task", custom).runtimeSeconds, 900);
  assert.equal(turnDeadlines("task", custom).watchdogMs, 910000);
