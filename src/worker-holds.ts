...
export class WorkerHolds {
  ...
  recordAction(w:Worker,c:Conversation,input:unknown,jobId:string){
    ...
    const digest=textField(e,'actionDigest'),reference=textField(e,'evidenceReference');
    if(!/^[a-f0-9]{64}$/.test(digest) || reference.length>1000)throw new Error('Exact action digest and bounded private evidence reference required');
    ...
    h.actionDigest=digest;h.evidenceReference=reference;h.recordedByJob=jobId;
